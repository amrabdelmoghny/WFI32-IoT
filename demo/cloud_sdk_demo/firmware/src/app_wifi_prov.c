        ...
        case APP_TCP_SERVER_PARSE_SOCKET_DATA:
        {
            ...
            if(readSize)
            {
                ...
                if (p != NULL && !strncmp(p, APP_WIFI_PROV_DONE_ID, strlen(APP_WIFI_PROV_DONE_ID))) {                                   
                    ...
                    if(0 != APP_WriteWifiConfigFile((char*)wifi.ssid, 
                                                    (char*)wifi.key, 
                                                    wifi.auth, 
                                                    true)){
                        APP_WIFI_PROV_DBG(SYS_ERROR_ERROR, "Failed to save credentials to USB MSD \r\n");
                    }
                    
                    APP_WIFI_PROV_PRNT("Provisioning complete\r\n");
                    SET_WIFI_CREDENTIALS(VALID_CREDENTIALS_SRC_AP,CREDENTIALS_VALID);
                    appWifiProvData.tcpServerTaskState = APP_TCP_SERVER_CLOSE_SOCKET;
                }
            }
